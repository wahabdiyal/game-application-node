...
export class UserService {
    ...
    if (search && date.length > 0 && role) {
      ...
      totalCount = await this.userModel
        .find({
          ...
          country: { $in: country.replace(' ', '').split(',') },
          role: 'player',
  ...
  async create(user: any): Promise<any> {
    ...
    let usercheck = await this.userModel.find({
      $or: [{ email: user.email }, { phone: user.phone }],
    });

    if (!usercheck.length) {
      let getCoinValue = await this.signuprewardService.getCoinByUserCountry(
        user.country,
      );
      if (user.refereal_code) {
        const getRefDetail = await this.refcodeService.getRefWithCode(
          user.refereal_code,
        );
        if (getRefDetail) {
          //   return getRefDetail;
          ...
          // return userRef;
          const refRewardSetting =
            await this.refrewardService.getRefRewardByDate();

          if (refRewardSetting && getRefDetail && userRef) {
            const daycount =
              moment(this.getDate()).diff(
                moment(
                  this.getDate(
                    moment(getRefDetail.use_date)
                      ? getRefDetail.use_date
                      : '2023-01-01 00:00:00',
                  ),
                ),
                'days',
            ...
            if (
              Number(refRewardSetting.days_limit) >= daycount &&
              Number(getRefDetail.total_use) <
                Number(refRewardSetting.referral_limit)
            ) {
              if (Number(refRewardSetting.silver_coin) > 0) {
                await this.UpdateUser(
                  getRefDetail.user_id,
                  Number(userRef['silver_balance']) +
                    Number(refRewardSetting.silver_coin),
                  'silver',
                );
                const response = await axiosInstance
                  .post(
                    'http://localhost:2053/silvers/apirequest/server/jk_y97wah',
                    {
              ...
              if (Number(refRewardSetting.gold_coin) > 0) {
                await this.UpdateUser(
                  getRefDetail.user_id,
                  Number(userRef['gold_balance']) +
                    Number(refRewardSetting.gold_coin),
                  'gold',
                );

                const response = await axiosInstance
                  .post(
                    'http://localhost:2053/golds/apirequest/server/jk_y97wah',
                    {
                      coins: refRewardSetting.silver_coin,
                      bal: refRewardSetting.silver_coin,
                      remarks: 'Added by refferal reward',
              ...
              await this.refcodeService.update(getRefDetail.id, {
                total_use: Number(getRefDetail.total_use) + 1,
                use_date: moment(new Date()).format('YYYY-MM-DD HH:mm:ss'),
              });
            }
          }
      ...
      const lastRecords = await this.userModel
        .find()
        .sort({ createdAt: -1 })
        .limit(1);
      ...
      const userVal = await this.userModel.create({
        ...user,
        silver_balance: getCoinValue?.silver_coin,
      ...
      if (getCoinValue) {
        if (
          Number(getCoinValue.gold_coin) != 0 &&
          Number(getCoinValue.silver_coin) != 0
        ) {
          await axiosInstance
            .post('http://localhost:2053/silvers/apirequest/server/jk_y97wah', {
          ...
          await axiosInstance
            .post('http://localhost:2053/golds/apirequest/server/jk_y97wah', {
  ...
  async findAllUserCount() {
    const operator = await this.userModel.countDocuments({
      role: 'operator',
      status: 'active',
    });
    const player = await this.userModel.countDocuments({
      role: 'player',
      status: 'active',
    });
  ...
  async getAllUser() {
    return await this.userModel
      .find({ $or: [{ role: 'Player' }, { role: 'player' }] })
  ...
  async verifyAdminToken(token) {
    const adminToken = await this.userModel.findOne({
