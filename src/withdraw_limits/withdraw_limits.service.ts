...
export class WithdrawLimitsService {
  ...
  async create(createWithdrawLimitDto: CreateWithdrawLimitDto) {
    const countries = await this.Withdraw_limitsModel.findOne({
      countries: { $in: createWithdrawLimitDto['countries'] },
    });
    if (countries) {
  ...
  async findAll(myRole = '', myCountries = '') {
    ...
    if (myRole != 'Admin' && myRole != 'admin')
      query['countries'] = { $in: myCountries.split(', ') };
  ...
  async findOneByCountry(country: string) {
    return await this.Withdraw_limitsModel.findOne({
      countries: { $in: [country] },
    });
  }
  async findOneByCountryMobile(country: string) {
    const data = await this.Withdraw_limitsModel.findOne({
      countries: { $in: [country] },
    });
    if (!data)
      return {
  ...
  async update(id: string, updateWithdrawLimitDto: UpdateWithdrawLimitDto) {
    const withdraw = await this.Withdraw_limitsModel.findByIdAndUpdate(
      id,
      updateWithdrawLimitDto,
    );
    if (!withdraw) {
