...
export class ChallengesService {
  ...
  async create(createChallengeDto: CreateChallengeDto) {
    ...
    const game = await this.gameModel.findOne({
      _id: createChallengeDto['game_id'],
    });

    ///////check for block status allow_to_game from `user` by challenger_id
    const player = await this.userModel.findOne({
      _id: createChallengeDto['challenger_id'],
    });
    if (
      !player.allow_to_game &&
      new Date(player.restriction_end_at) > new Date()
    ) {
      ...
      const timeDifferenceM: any =
        (unBlock_time.getTime() - new Date().getTime()) / 60000;
      return 'challenges blocked for ' + timeDifferenceM + ' mints';
    } else {
      await this.un_restrictUserForChallenges(
        createChallengeDto['challenger_id'],
      );
    }

    /////check for maximum challenges limit
    const challenge = await this.checkForChallenges(
      createChallengeDto['challenger_id'],
      parseInt(game.maximum_challenges),
    );
    if (challenge === true) return 'exceed maximum challenges please wait..';
  ...
  async checkForChallenges(user_id: string, maximum_limits: number) {
    const total_challenges = await this.challengeModel.countDocuments({
      challenger_id: user_id,
    });
    if (maximum_limits < total_challenges) return true;
    else return false;
    // return count > maximum_limits;
  }

  async restrictUserForChallenges(
    user_id: string,
    restriction_time_mints: number,
  ) {
    await this.userModel.updateOne(
      { _id: user_id },
      {
        ...
        restriction_end_at: new Date(
          new Date().getTime() + restriction_time_mints * 60000,
        ),
      },
    );
