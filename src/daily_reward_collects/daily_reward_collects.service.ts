...
export class DailyRewardCollectsService {
  ...
  async create(user) {
    ...
    const rewardDetail = await this.dailyRewardService.findUserCountry(
    ...
    if (!rewardDetail) {
      return {
        status: false,
        message: 'Country is not available for this user.',
      };
    }
    const rewardCollect = await this.dailyRewardCollectionModel.findOne({
      user_id: user.id,
    });
    ...
    if (!rewardCollect) {
      if (rewardDetail.gold_coin != '0') {
        await this.goldService.create({
          coins: rewardDetail.gold_coin,
          type: 'credit',
          remarks: 'Daily reward collect',
          entry_type: 'admin',
          client_id: user.id,
          transaction_id: rewardDetail['_id'],
          transaction_status: 'daily reward',
        });
      }
      if (rewardDetail.silver_coin != '0') {
        await this.silverService.create({
          coins: rewardDetail.silver_coin,
          type: 'credit',
          remarks: 'Daily reward collect',
          entry_type: 'admin',
          client_id: user.id,
      ...
      const dailyre = await this.dailyRewardCollectionModel.create({
        user_id: user.id,
        total_reward: rewardDetail.inactive_day,
        reward_count: 1,
        date: moment(),
      ...
      return {
        ...
        dailyReward: {
          ...dailyre.toObject(),
          silver: rewardDetail.silver_coin,
          gold: rewardDetail.gold_coin,
        },
        message: 'Added User Daily reward',
      };
    } else {
  ...
  async checkCollectUser(user): Promise<boolean> {
    const value = await this.dailyRewardCollectionModel.findOne({
      user_id: user.id,
    });
    return value == null ? true : false;
  }
  async getCollectUser(user) {
    const dailyCollection = await this.dailyRewardCollectionModel.findOne({
      user_id: user.id,
    });
    if (dailyCollection) {
      return {
        status: true,
        message: 'User Daily reward collection',
        reward: dailyCollection,
      };
    } else {
  ...
  async sendRequest() {
    this.eventEmitter.emit('daily.rewards', new DailyRewardCollect());
  ...
  async eventDailyReward(payload: DailyRewardCollect) {
    ...
    for (let c = 0; c < userDailyReward.length; c++) {
      const rewardDetailAdmin = await this.dailyRewardService.findByCountry(
        userDailyReward[c].country,
      );
      //  current date match//
      const today = moment();
      ...
      if (
        Number(userDailyReward[c].reward_count) <
        Number(rewardDetailAdmin.inactive_day)
      ) {
        ///////////////////////// update daily reward package update//////////////////
        if (rewardDetailAdmin.gold_coin != '0') {
          await this.goldService.create({
            coins: rewardDetailAdmin.gold_coin,
            type: 'credit',
            remarks: 'Daily reward collect',
            entry_type: 'admin',
            client_id: userDailyReward[c].user_id,
            transaction_id: rewardDetailAdmin['_id'],
            transaction_status: 'daily reward',
          });
        }
        if (rewardDetailAdmin.silver_coin != '0') {
          await this.silverService.create({
            coins: rewardDetailAdmin.silver_coin,
            type: 'credit',
            remarks: 'Daily reward collect',
            entry_type: 'admin',
            client_id: userDailyReward[c].user_id,
        ...
        await this.dailyRewardCollectionModel.updateOne(
          { user_id: userDailyReward[c].user_id },
          {
            reward_count: Number(userDailyReward[c].reward_count) + 1,
            date: moment(),
            country: userDailyReward[c].country,
          },
        );
      } else {
        console.log('continue');
