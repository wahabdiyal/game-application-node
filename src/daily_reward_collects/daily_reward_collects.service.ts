  ...
  async create(user) {
    ...
    if (!rewardCollect) {
      if (rewardDetail.gold_coin != '0') {
        await this.goldService.create({ coins: rewardDetail.gold_coin, type: "credit", remarks: "Daily reward collect", entry_type: "admin", client_id: user.id, transaction_id: rewardDetail['_id'], transaction_status: "daily reward" })
      }
      if (rewardDetail.silver_coin != '0') {
        await this.silverService.create({ coins: rewardDetail.silver_coin, type: "credit", remarks: "Daily reward collect", entry_type: "admin", client_id: user.id, transaction_id: rewardDetail['_id'], transaction_status: "daily reward" })
      }
      ...
      return {
        ...await this.userService.getUserRenewTokenForMobile(user['id']), dailyReward: { ...dailyre.toObject(), silver: rewardDetail.silver_coin, gold: rewardDetail.gold_coin }, message: "Added User Daily reward"
    ...
    for (let c = 0; c < userDailyReward.length; c++) {
      ...
      if (Number(userDailyReward[c].reward_count) < Number(rewardDetailAdmin.inactive_day)) {
        ...
        if (rewardDetailAdmin.gold_coin != '0') {
          await this.goldService.create({ coins: rewardDetailAdmin.gold_coin, type: "credit", remarks: "Daily reward collect", entry_type: "admin", client_id: userDailyReward[c].user_id, transaction_id: rewardDetailAdmin['_id'], transaction_status: "daily reward" })
        }
        if (rewardDetailAdmin.silver_coin != '0') {
          await this.silverService.create({ coins: rewardDetailAdmin.silver_coin, type: "credit", remarks: "Daily reward collect", entry_type: "admin", client_id: userDailyReward[c].user_id, transaction_id: rewardDetailAdmin['_id'], transaction_status: "daily reward" })
        }
