...
export class BetsService {
  ...
  async create(createbetDto: CreateBetDto) {
    ...
    const checkBetId = await this.betsModel.findOne({
    ...
    const first_user = await this.userService.findUserbyId(
      createbetDto['first_player'],
    );
    ...
    if (
      ...
      for (let c = 0; c < first_user['bet_block'].length; c++) {
        ...
        if (element.game == game.game_id) {
          const today = moment();
          const block_date = moment(element.date).add(
            game.time_restrictions,
            'minutes',
          );
      ...
      if (!bet_block_status) {
        ...
        return {
          status: false,
          message:
            'User is blocked for this game.Remain time for Unblock:' +
            this.roundTwoDecimalPlaces(diff) +
            ' minutes.',
        };
      }
    }
  ...
  async createBetForFirstAndSecondUser(createbetDto: CreateBetDto) {
    ...
    if (
      createbetDto['second_player'] === undefined &&
      createbetDto['second_player'] === undefined
    ) {
      return {
        status: false,
        message: 'Challenges must for two player Second Player not found.',
      };
    }
    const first_user = await this.userService.findUserbyId(
      createbetDto['first_player'],
    );
    const second_user = await this.userService.findUserbyId(
      createbetDto['second_player'],
    );
  ...
  async remove(id: any) {
    try {
      const betDetail = await this.betsModel
        .findOne({ _id: id })
        .populate('game_id')
      ...
      await this.sendNotificationToUser(
        ...
        'Current challenge has been removed.',
        'challengedeleted',
      );
      if (betDetail.second_player && betDetail.second_player != '') {
        await this.sendNotificationToUser(
          betDetail.second_user_id,
          'Current challenge has been removed.',
          'challengedeleted',
        );
      }
  ...
  async betUpdateLoseWin(id: string, status: boolean) {
    ...
    if (bet && bet.status != 'complete') {
      ...
      return await this.userService.updateMobile(user['id'], {
        silver_balance: Number(user['silver_balance']),
      });
    } else {
  ...
  async betSecondSilverUser(id, second_user, title, message) {
    ...
    if (bet && bet.status == 'inactive' && Number(bet.silver) > 0) {
      ...
      if (user && Number(user['silver_balance']) > Number(bet['silver'])) {
        ...
        return {
          status: false,
          message: 'Bet amount is higher than account balance',
        };
      }
    } else {
  ...
  async findAll(
    ...
    if (myRole != 'Admin' && myRole != 'admin')
      query['first_user_country'] = { $in: myCountries.split(', ') };
    if (date.length > 0) {
    ...
    let totalCount = await this.betsModel
      .find(query)
      .sort({ createdAt: -1 })
      .countDocuments()
      .exec();
  ...
  async ignore_count(id) {
    ...
    if (bet && bet.second_player != '') {
      if (
        Number(bet.game_id['ignore_bet']) - 1 >=
        Number(bet['ignore_count'])
      ) {
        await this.sendNotificationToUser(
          bet.second_user_id,
          'Sorry Player ' +
        ...
        await this.betsModel.updateOne(
          { _id: id },
          {
            ignore_count: Number(bet['ignore_count']) + 1,
            second_player: '',
            second_email: '',
            second_user_country: '',
            status: 'inactive',
        ...
        if (bet_block) {
          await this.userService.update(bet.first_player['_id'], {
            bet_block: bet.first_player['bet_block'].concat({
              game: bet.game_id['game_id'],
        ...
        await this.sendNotificationToUser(
          bet.second_user_id,
          'Your opponent has been blocked for this challenge. ',
          'oponentblocked',
        );
  ...
  async acceptBet(id) {
    ...
    if (bet && bet.status == 'inprocess') {
      ...
      if (Number(bet['silver'])) {
        await this.userService.UpdateUser(
          user['id'],
          Number(user['silver_balance']) - Number(bet['silver']),
          'silver',
        );
      } else if (Number(bet['gold'])) {
        await this.userService.UpdateUser(
          user['id'],
          Number(user['gold_balance']) - Number(bet['gold']),
          'gold',
        );
      } else {
      ...
      return {
        ...(await this.userService.getUserRenewTokenForMobile(
          bet.first_player['_id'],
        )),
        main_player_info: updateBet['main_player_info'],
        bet: updateBet,
        game: await this.gameService.findOne(updateBet['game_id']),
      };
    }
    ...
    if (bet && bet.second_player != '') {
      if (Number(bet.game_id['reject_bet']) >= Number(bet['reject_counter'])) {
        ...
        await this.betsModel.updateOne(
          { _id: id },
          {
            reject_counter: Number(bet['reject_counter']) + 1,
            second_player: '',
            second_email: '',
            second_user_country: '',
            status: 'inactive',
  ...
  async rejectBySecondUser(id) {
    ...
    if (bet && bet.second_player != '' && bet.status == 'inprocess') {
      ...
      await this.betsModel.updateOne(
        { _id: id },
        { reject_counter: Number(bet['reject_counter']) + 1, status: 'reject' },
      );
      return { status: true, message: 'Bet reject updated successfully.' };
    } else {
      return { status: false, message: 'No bet found.' };
  ...
  async reverseBet(id) {
    ...
    if (bet && bet.status != 'cancel') {
      ...
      if (bet.silver != '0') {
        await this.userService.UpdateUser(
          first_user['id'],
          Number(first_user['silver_balance']) + Number(bet['silver']),
          'silver',
        );
      }
      if (bet.gold != '0') {
        await this.userService.UpdateUser(
          first_user['id'],
          Number(first_user['gold_balance']) + Number(bet['gold']),
          'gold',
        );
      }
      ...
      if (bet.second_player) {
        ...
        if (bet.silver != '0') {
          await this.userService.UpdateUser(
            second_user['id'],
            Number(second_user['silver_balance']) + Number(bet['silver']),
            'silver',
          );
        }
        if (bet.gold != '0') {
          await this.userService.UpdateUser(
            second_user['id'],
            Number(second_user['gold_balance']) + Number(bet['gold']),
            'gold',
          );
        }
      }
  ...
  async sendRequest() {
    this.eventEmitter.emit('bet-expire');
    console.log('send bet request.....');
  }
  @OnEvent('bet-expire')
  async eventDailyReward() {
    const bets: any = await this.betsModel
    ...
    for (let c = 0; c < bets.length; c++) {
      //  current date match//
      const today = moment();
      const startDate = moment(bets[c].updatedAt).add(
        bets[c].game_id.challenge_time_minutes,
        'minutes',
      );

      if (startDate.isBefore(today)) {
        await this.betsModel.findByIdAndUpdate(bets[c]._id, {
          status: 'expired',
        });
        if (Number(bets[c].silver)) {
          await this.sendNotificationToUser(
            bets[c].first_player.userId,
        ...
        if (Number(bets[c].gold)) {
          await this.userService.UpdateUser(
            bets[c].first_player._id,
            Number(bets[c].first_player.gold_balance) + Number(bets[c].gold),
            'gold',
          );
          await this.sendNotificationToUser(
            bets[c].first_player.userId,
