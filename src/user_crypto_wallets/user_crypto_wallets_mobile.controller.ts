...
export class UserCryptoWalletsMobileController {
  ...
  async create(@Body() createUserCryptoWalletDto: CreateUserCryptoWalletDto) {
    const cryptoDetail = await this.userCryptoWalletsService.create(createUserCryptoWalletDto);
  ...
  @Patch(':id')
  async update(@Param('id') id: string, @Body() updateUserCryptoWalletDto: UpdateUserCryptoWalletDto) {
    
    return await this.userCryptoWalletsService.update(id, updateUserCryptoWalletDto);
  }
