...
export class CreateLogsController {
  ...
  @Get('/')
  async getUser(@Request() req, @Query() { page, perpage, search, start_date, end_date, countryName }) {
    let date = (start_date && end_date) ? [{ start: start_date, end: end_date }] : [];
    return this.createLogsService.findAll(page, perpage, date, search, countryName, req.user.role, req.user.country);
  }
